$main.controller('diamondsCountController',diamondsCountController)
$main.config(function($stateProvider, $urlRouterProvider){
	$stateProvider.state('main.consumedata.diamondsCount',{//钻石统计
    	url:'/diamondsCount',
    	templateUrl:'template/hie_idp/diamondsCount.html?T='+Math.random(),
    	controller:'diamondsCountController'
    })
	
})
function diamondsCountController($scope,$http,T){
	...
	$scope.queryDiamondsStatList=function(){
		$http.post("hieConsume/queryMoneyStatList.action",$scope.postData).success(function(data){
			if(data.status!= 1){
				data.result=[];
			}
			var limit = 10
