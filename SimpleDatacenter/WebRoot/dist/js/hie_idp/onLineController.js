$main.controller('onLineController',onLineController)
$main.config(function($stateProvider, $urlRouterProvider){
	$stateProvider.state('main.yunyindata.onLine',{//任务打点
    	url:'/onLine',
    	templateUrl:'template/hie_idp/onLine.html?T='+Math.random(),
    	controller:'onLineController'
    })
	
})
function onLineController($scope,$http,T){
	...
	$scope.queryTaskStatList=function(){
		$http.post("hieOperate/queryOnlineList.action",$scope.postData).success(function(data){
			if(data.status!= 1){
				data.result=[];
			}
