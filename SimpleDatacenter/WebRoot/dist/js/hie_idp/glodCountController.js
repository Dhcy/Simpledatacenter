$main.controller('glodCountController',glodCountController)
$main.config(function($stateProvider, $urlRouterProvider){
	$stateProvider.state('main.consumedata.goldCount',{//金币统计
    	url:'/goldCount',
    	templateUrl:'template/hie_idp/glodCount.html?T='+Math.random(),
    	controller:'glodCountController'
    })
	
})
function glodCountController($scope,$http,T){
	...
	$scope.queryDiamondsStatList=function(){
		$http.post("hieConsume/queryGoldStatList.action",$scope.postData).success(function(data){
			console.log(data);
			if(data.status!= 1){
				data.result=[];
			}
