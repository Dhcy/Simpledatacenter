$main.controller('taskPointController',taskPointController)
$main.config(function($stateProvider, $urlRouterProvider){
	$stateProvider.state('main.yunyindata.taskPoint',{//任务打点
    	url:'/taskPoint',
    	templateUrl:'template/hie_idp/taskPoint.html?T='+Math.random(),
    	controller:'taskPointController'
    })
	
})
function taskPointController($scope,$http,T){
	...
	$scope.queryTaskStatList=function(){
		$http.post("hieOperate/queryTaskStatList.action",$scope.postData).success(function(data){
			if(data.status!= 1){
				data.result=[];
			}
