function playersChumController($scope,$http,T){
	$scope.graphfun={		
		...
		"dataList_1_2_graph":function(data,xdata,ydata){
			...
			//						symbol: 'circle',
						...
						series: [{
							name: $scope.panel_1.dataList_2.th.type[$scope.palyerType],
	...
	$scope.panel_1={	
		...
		'dataList_2':{
			...
			'orderBy':{'s':'','t':''},
			'th':{
				'type':{"-1":T.T("全部玩家数(角色)"),"1":T.T("付费玩家数(角色)"),"0":T.T("非付费玩家数(角色)")},
			}
		}
	};
	$scope.panel_2={	
	...
	$scope.getAllData=function(){
		...
		$http.post("players/playersChurn/selectPerDayLostNumAndRate.action",$scope.searchData).success(function(data, status, headers, config){
			console.log(data)
			//alert();
		...
		$http.post("players/playersChurn/selectPerDayReturnNumList.action",$scope.searchData).success(function(data, status, headers, config){
			console.log(data)
			//alert();
	...
	$scope.getChumPlayerData=function(){
		...
		$http.post("players/playersChurn/selectLostUserAnalysNumList.action",$scope.searchData2).success(function(data, status, headers, config){
