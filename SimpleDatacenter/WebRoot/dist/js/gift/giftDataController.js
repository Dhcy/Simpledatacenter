function giftDataController($scope,$http,T){
	$scope.graphfun={
		...
		$('.panel_2 .dataList_1 .graph').highcharts({
					...
					xAxis: {
						type: 'linear',
						tickmarkPlacement: 'on',
						categories: ($scope.dataPushArr( $scope.panel_2.dataList_1.data,'giftName')).newArr
					},
					yAxis: {
						title:"",
						labels: {
	...
	$scope.giftDrawing = function(){
		$http.post("gift/giftReceive.action",$scope.searchData).success(function(data,status,headers,config){
			console.log(data);
			if(data == undefined){
				data = [];
			}
		...
		/*$http.post("testData/new.json?date="+$scope.mstatdate+"&channel="+$scope.mchannel+"&qf="+$scope.mareaclothing+"&t="+Math.random(),{}).success(function(data, status, headers, config){
	...
	$scope.giftDrawingDetail = function(){
		$http.post("gift/giftReceiveDetail.action",$scope.searchData).success(function(data,status,headers,config){
			console.log(data);
			if(data == undefined){
				data = [];
			}
