 ...
 * 
 */
function sdkdataNewAddController($scope,$http,T){
	$scope.panel_1 = {
	        'model': 'table',
	        'gameList':[],
	        'channelList':[],
...
// 	    $scope.postData2.enddate = $scope.mstatdate.indexOf("~")>0?$scope.mstatdate.split("~")[1].trim():$scope.mstatdate;
 	   ...
 	   //
 	   $http.post("sdkdata/selectSDKDataNewAddDataDetail.action",$scope.postData2).success(function(data){
	...
	$scope.getData=function(){
		...
		$http.post("sdkdata/selectSDKDataNewAddData.action",$scope.postData).success(function(data){
			console.log(data)
			if(data == undefined || data == null){
				data = [];
			}
