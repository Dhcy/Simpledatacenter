 ...
 * 
 */
function sdkdataBrowserController($scope,$http,T){
	$scope.panel_1 = {
	        'model': 'table',
	        'gameList':[],
	        'channelList':[],
	...
	$scope.getDataDetail = function(row){
		...
		$scope.postData2.categoryId = row.category_id;
		$scope.postData2.pv = row.pv;
		$scope.postData2.uv = row.uv;
 	   //
 	   $http.post("sdkdata/selectSDKBrowseSummaryDetail.action",$scope.postData2).success(function(data){
	...
	$scope.getData=function(){
		...
		$http.post("sdkdata/selectSDKBrowseSummary.action",$scope.postData).success(function(data){
			console.log(data)
			if(data == undefined || data == null){
				data = [];
			}
