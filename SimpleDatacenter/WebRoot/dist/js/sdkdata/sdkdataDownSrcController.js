 ...
 * 
 */
function sdkdataDownSrcController($scope,$http,T){
	$scope.panel_1 = {
	        'model': 'table',
	        'gameList':[],
	        'channelList':[],
...
// 	    $scope.postData2.enddate = $scope.mstatdate.indexOf("~")>0?$scope.mstatdate.split("~")[1].trim():$scope.mstatdate;
		$scope.postData2.startdate = $scope.postData2.enddate = row.statdate;
 	    ...
 	    $scope.postData2.gameid = row.gameid;
 	   //
 	   $http.post("sdkdata/selectSDKDownloasdSourceDetail.action",$scope.postData2).success(function(data){
	...
	$scope.getData=function(){
		...
		$http.post("sdkdata/selectSDKDownloasdSourceSummary.action",$scope.postData).success(function(data){
			console.log(data)
			if(data == undefined || data == null){
				data = [];
			}
