$main.controller('czzpropCountController',czzpropCountController)
$main.config(function($stateProvider, $urlRouterProvider){
	$stateProvider.state('main.consumedata.czzpropCount',{//道具统计
    	url:'/czzpropCount',
    	templateUrl:'template/czz_idp/propCount.html?T='+Math.random(),
    	controller:'czzpropCountController'
    })
	
})
function czzpropCountController($scope,$http,T){
	...
	$scope.queryDiamondsStatList=function(){
		$http.post("czzCost/queryThingStatList.action",$scope.postData).success(function(data){
			if(data.status!= 1){
				data.result=[];
			}
			var limit = 10
