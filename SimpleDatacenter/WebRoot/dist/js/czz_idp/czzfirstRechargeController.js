$main.controller('czzfirstRechargeController',czzfirstRechargeController)
$main.config(function($stateProvider, $urlRouterProvider){
	$stateProvider.state('main.yunyindata.czzfirstRecharge',{//首冲等级
    	url:'/czzfirstRecharge',
    	templateUrl:'template/czz_idp/firstRecharge.html?T='+Math.random(),
    	controller:'czzfirstRechargeController'
    })
	
})
function czzfirstRechargeController($scope,$http,T){
	...
	$scope.queryTaskStatList=function(){
		$http.post("czzOperate/queryFirstRechargeStatList.action",$scope.postData).success(function(data){
			if(data.status!= 1){
				data.result=[];
			}
