$main.controller('czztaskPointController',czztaskPointController)
$main.config(function($stateProvider, $urlRouterProvider){
	$stateProvider.state('main.yunyindata.czztaskPoint',{//任务打点
    	url:'/czztaskPoint',
    	templateUrl:'template/czz_idp/taskPoint.html?T='+Math.random(),
    	controller:'czztaskPointController'
    })
	
})
function czztaskPointController($scope,$http,T){
	...
	$scope.selectGameAreaList = function(){
	 ...
	 $scope.styleList=[{id:1,name:"主线"},{id:2,name:"支线"}];
	
	
	$scope.queryTaskStatList=function(){
		$http.post("czzOperate/queryTaskStatList.action",$scope.postData).success(function(data){
			if(data.status!= 1){
				data.result=[];
			}
