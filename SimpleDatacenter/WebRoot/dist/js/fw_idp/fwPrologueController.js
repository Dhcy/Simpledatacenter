$main.controller('fwPrologueController',fwPrologueController)
$main.config(function($stateProvider, $urlRouterProvider){
	$stateProvider.state('main.indpd.fwprologuecount',{//关卡查询
    	url:'/fwprologuecount',
    	templateUrl:'template/fw_idp/fwprologue.html?T='+Math.random(),
    	controller:'fwPrologueController'
    })
 ...
 * @return
 */
function fwPrologueController($scope,$http,T){
	...
	$scope.selectPrologueList = function(){        
		$http.post("fwSummary/selectPrologueList.action",$scope.searchData).success(function(data){
			if(data.prologueList == undefined||data.prologueList==null){
				data.prologueList =[];
			}
            $scope.panel_1.dataList_1.data=data.prologueList;
