$main.controller('czzectypeCountController',czzectypeCountController)
$main.config(function($stateProvider, $urlRouterProvider){
	$stateProvider.state('main.consumedata.czzectypeCount',{//副本统计
    	url:'/czzectypeCount',
    	templateUrl:'template/czz_idp/ectypeCount.html?T='+Math.random(),
    	controller:'czzectypeCountController'
    })
	
})
function czzectypeCountController($scope,$http,T){
	...
	$scope.queryDiamondsStatList=function(){
		$http.post("czzCost/queryEctypeStatList.action",$scope.postData).success(function(data){
			if(data.status!= 1){
				data.result=[];
			}
