$main.controller('czzonTimeRechargeController',czzonTimeRechargeController)
$main.config(function($stateProvider, $urlRouterProvider){
	$stateProvider.state('main.yunyindata.czzonTimeRecharge',{//按时等级
    	url:'/czzonTimeRecharge',
    	templateUrl:'template/czz_idp/onTimeRecharge.html?T='+Math.random(),
    	controller:'czzonTimeRechargeController'
    })
	
})
function czzonTimeRechargeController($scope,$http,T){
	...
	$scope.queryTaskStatList=function(){
		$http.post("czzOperate/queryOnTimeRechargeList.action",$scope.postData).success(function(data){
			console.log(data)
			if(data.status!= 1){
				data.result=[];
			}
