$main.controller('czzdiamondsCountController',czzdiamondsCountController)
$main.config(function($stateProvider, $urlRouterProvider){
	$stateProvider.state('main.consumedata.czzdiamondsCount',{//货币消耗&存量统计
    	url:'/czzdiamondsCount',
    	templateUrl:'template/czz_idp/diamondsCount.html?T='+Math.random(),
    	controller:'czzdiamondsCountController'
    })
	
})
function czzdiamondsCountController($scope,$http,T){
    ...
    $scope.styleList=[{id:1,name:T.T("消耗")},{id:2,name:T.T("产出")}];
	...
	$scope.queryDiamondsStatList=function(){
		$http.post("czzCost/queryMoneyStatList.action",$scope.postData).success(function(data){
			if(data.status!= 1){
				data.result=[];
			}
			var limit = 10
