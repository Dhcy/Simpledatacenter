$main.controller('czzshoppingCountController',czzshoppingCountController)
$main.config(function($stateProvider, $urlRouterProvider){
	$stateProvider.state('main.consumedata.czzshoppingCount',{//商城统计
    	url:'/czzshoppingCount',
    	templateUrl:'template/czz_idp/shoppingCount.html?T='+Math.random(),
    	controller:'czzshoppingCountController'
    })
	
})
function czzshoppingCountController($scope,$http,T){
	...
	$scope.queryTaskStatList=function(){
		$http.post("czzCost/queryShoppingStatList.action",$scope.postData).success(function(data){
			if(data.status!= 1){
				data.result=[];
			}
			var limit = 10
