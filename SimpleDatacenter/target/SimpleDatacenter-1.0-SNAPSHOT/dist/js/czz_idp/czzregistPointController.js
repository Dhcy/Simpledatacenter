$main.controller('czzregistPointController',czzregistPointController)
$main.config(function($stateProvider, $urlRouterProvider){
	$stateProvider.state('main.yunyindata.czzregistPoint',{//注册打点
    	url:'/czzregistPoint',
    	templateUrl:'template/czz_idp/registPoint.html?T='+Math.random(),
    	controller:'czzregistPointController'
    })
	
})
function czzregistPointController($scope,$http,T){
	$scope.panel_1={
			'model':'table',
			'dataList_1':{
				'data':'',
	...
	$scope.queryRegistStatList=function(){
		$http.post("czzOperate/queryRegistStatList.action",$scope.postData).success(function(data){
			if(data.status!= 1){
				data.result=[];
			}
