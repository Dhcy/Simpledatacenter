 ...
 * @return
 */
function financeGameUserController($scope,$http,T){
	$scope.panel_1={
			'model':'table',
			'dataList_1':{
				'data':'',
	...
	$scope.selectFinanceGameUserNumList=function(){
		$http.post("financeData/selectFinanceGameUserNumList.action",$scope.postData).success(function(data){
			if(data.gameUserNumList== undefined||data.gameUserNumList==null){
				data.gameUserNumList=[];
			}
			if(data.total== undefined||data.total==null){
				data.total={'statdate':'','gameName':'','newUser':'','price':'','money':''};
			}
			$scope.panel_1.dataList_1.data=data.gameUserNumList;
			$scope.panel_1.dataList_1.total=data.total;
