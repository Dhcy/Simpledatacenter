function gameversionModifyController($scope, $http, $stateParams, $state,T) {
	...
	$scope.postData = function() {
		if ($scope.ctype == 1) {	//添加
			$http.post("game/addGameVersion.action", $scope.userData).success(function(data) {
				if (data.status == 0) {
					alert(data.message);
				} else {
					$scope.backGo();
	...
	$scope.backGo = function() {
		$state.go('main.config.gameversion');
	}
