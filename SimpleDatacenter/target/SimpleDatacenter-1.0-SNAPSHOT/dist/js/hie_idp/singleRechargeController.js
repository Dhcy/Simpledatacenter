$main.controller('singleRechargeController',singleRechargeController)
$main.config(function($stateProvider, $urlRouterProvider){
	$stateProvider.state('main.yunyindata.singleRecharge',{//首冲等级
    	url:'/singleRecharge',
    	templateUrl:'template/hie_idp/singleRecharge.html?T='+Math.random(),
    	controller:'singleRechargeController'
    })
	
})
function singleRechargeController($scope,$http,T){
	...
	$scope.queryTaskStatList=function(){
		$http.post("hieOperate/querySingleRechargeList.action",$scope.postData).success(function(data){
			console.log(data)
			if(data.status!= 1){
				data.result=[];
			}
