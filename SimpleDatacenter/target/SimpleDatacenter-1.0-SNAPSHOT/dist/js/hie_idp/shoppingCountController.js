$main.controller('shoppingCountController',shoppingCountController)
$main.config(function($stateProvider, $urlRouterProvider){
	$stateProvider.state('main.consumedata.shoppingCount',{//任务打点
    	url:'/shoppingCount',
    	templateUrl:'template/hie_idp/shoppingCount.html?T='+Math.random(),
    	controller:'shoppingCountController'
    })
	
})
function shoppingCountController($scope,$http,T){
	...
	$scope.queryTaskStatList=function(){
		$http.post("hieConsume/queryShoppingStatList.action",$scope.postData).success(function(data){
			if(data.status!= 1){
				data.result=[];
			}
			var limit = 10
