$main.controller('onTimeRechargeController',onTimeRechargeController)
$main.config(function($stateProvider, $urlRouterProvider){
	$stateProvider.state('main.yunyindata.onTimeRecharge',{//首冲等级
    	url:'/onTimeRecharge',
    	templateUrl:'template/hie_idp/onTimeRecharge.html?T='+Math.random(),
    	controller:'onTimeRechargeController'
    })
	
})
function onTimeRechargeController($scope,$http,T){
	...
	$scope.queryTaskStatList=function(){
		$http.post("hieOperate/queryOnTimeRechargeList.action",$scope.postData).success(function(data){
			console.log(data)
			if(data.status!= 1){
				data.result=[];
			}
