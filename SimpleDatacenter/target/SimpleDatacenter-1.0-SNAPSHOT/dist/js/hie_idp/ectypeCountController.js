$main.controller('ectypeCountController',ectypeCountController)
$main.config(function($stateProvider, $urlRouterProvider){
	$stateProvider.state('main.consumedata.ectypeCount',{//金币统计
    	url:'/ectypeCount',
    	templateUrl:'template/hie_idp/ectypeCount.html?T='+Math.random(),
    	controller:'ectypeCountController'
    })
	
})
function ectypeCountController($scope,$http,T){
	...
	$scope.queryDiamondsStatList=function(){
		$http.post("hieConsume/queryEctypeStatList.action",$scope.postData).success(function(data){
			if(data.status!= 1){
				data.result=[];
			}
