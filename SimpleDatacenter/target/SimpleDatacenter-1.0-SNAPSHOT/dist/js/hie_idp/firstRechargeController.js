$main.controller('firstRechargeController',firstRechargeController)
$main.config(function($stateProvider, $urlRouterProvider){
	$stateProvider.state('main.yunyindata.firstRecharge',{//首冲等级
    	url:'/firstRecharge',
    	templateUrl:'template/hie_idp/firstRecharge.html?T='+Math.random(),
    	controller:'firstRechargeController'
    })
	
})
function firstRechargeController($scope,$http,T){
	...
	$scope.queryTaskStatList=function(){
		$http.post("hieOperate/queryFirstRechargeStatList.action",$scope.postData).success(function(data){
			if(data.status!= 1){
				data.result=[];
			}
