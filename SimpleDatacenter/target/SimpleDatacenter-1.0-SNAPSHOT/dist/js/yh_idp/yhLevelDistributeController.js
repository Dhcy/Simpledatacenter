$main.controller('yhLevelDistributeController',yhLevelDistributeController)
$main.config(function($stateProvider, $urlRouterProvider){
	$stateProvider.state('main.indpd.yhLevelDistribute',{//等级分布
    	url:'/yhLevelDistribute',
    	templateUrl:'template/yh_idp/yhLevelDistribute.html?T='+Math.random(),
    	controller:'yhLevelDistributeController'
    })
 ...
 * @return
 */
function yhLevelDistributeController($scope,$http,T){
	...
	$scope.selectLevelDistributeList=function(){
		$http.post("yhSummary/selectYhLevelDistributeList.action",$scope.postData).success(function(data){
			if(data== undefined||data==null){
				data=[];
			}
    ...
    $scope.getAllData=function(){
    	...
    	$scope.selectLevelDistributeList();
    }
  //加载下拉框
    $scope.getSelect();
    $scope.getAllData();
