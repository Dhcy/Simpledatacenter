$main.controller('yhSocialContractController',yhSocialContractController)
$main.config(function($stateProvider, $urlRouterProvider){
	$stateProvider.state('main.indpd.yhSocialContract',{//社交情况
    	url:'/yhSocialContract',
    	templateUrl:'template/yh_idp/yhSocialContract.html?T='+Math.random(),
    	controller:'yhSocialContractController'
    })
	
})
function yhSocialContractController($scope,$http,T){
	...
	$scope.selectActUserNumOfFriendsList=function(){
		$http.post("yhSocialContract/selectActUserNumOfFriendsList.action",$scope.postData).success(function(data){
			if(data== undefined||data==null){
				data=[];
			}
    ...
    $scope.getAllData=function(){
    	...
    	$scope.selectActUserNumOfFriendsList();
    }
  //加载下拉框
    $scope.getSelect();
    $scope.getAllData();
