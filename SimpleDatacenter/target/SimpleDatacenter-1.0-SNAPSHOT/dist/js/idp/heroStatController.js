$main.controller('heroStatController',heroStatController)
$main.config(function($stateProvider, $urlRouterProvider){
	$stateProvider.state('main.indpd.heroStat',{
    	url:'/heroStat',
    	templateUrl:'template/idp/heroStat.html?T='+Math.random(),
    	controller:'heroStatController'
    })
 ...
 * @return
 */
function heroStatController($scope,$http,T){
	...
	$scope.selectStageStatBoforePerday = function(){        
		$http.post("summary/selectStageStatBoforePerday.action",$scope.postData).success(function(data){
			if(data== undefined||data==null){
				data=[]
			}
	...
	$scope.selectHeroStageStatList = function(){        
		$http.post("summary/selectHeroStageStatList.action",$scope.postData).success(function(data){
			if(data== undefined||data==null){
				data=[]
			}
    ...
    $scope.getAllData=function(){
    	...
    	$scope.selectStageStatBoforePerday();
    	$scope.selectHeroStageStatList();
    }
  //加载下拉框
    $scope.getSelect();
    $scope.getAllData();
