 ...
 * 
 */
function sdkdataDownloadController($scope,$http,T){
	$scope.panel_1 = {
	        'model': 'table',
	        'gameList':[],
	        'channelList':[],
	...
	$scope.getDataDetail = function(row){
 	   ...
 	   //
 	   $http.post("sdkdata/selectSDKDownloadSummary.action",$scope.postData2).success(function(data){
	...
	$scope.getData=function(){
		...
		$http.post("sdkdata/selectSDKDownloadSummary.action",$scope.postData).success(function(data){
			console.log(data)
			if(data == undefined || data == null){
				data = [];
			}
